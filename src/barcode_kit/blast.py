from __future__ import annotations

import shutil
import subprocess
import tempfile
from collections import defaultdict
from collections.abc import Sequence
from dataclasses import dataclass, field
from pathlib import Path

from Bio.Seq import Seq

from barcode_kit.config import BlastRescueConfig
from barcode_kit.models import Marker
from barcode_kit.parser import format_fasta_record


__all__ = [
    "BlastRecord",
    "BlastRescueResult",
    "BlastRunner",
]


MAKEBLASTDB_COMMAND = "makeblastdb"
BLASTN_COMMAND = "blastn"
BLASTN_OUTFMT = "6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore"


@dataclass(frozen=True)
class BlastRecord:
    accession_version: str
    sequence: Seq | str


@dataclass(frozen=True)
class BlastHit:
    query_id: str
    subject_id: str
    identity: float
    alignment_length: int
    qstart: int
    qend: int
    sstart: int
    send: int
    evalue: float
    bitscore: float
    mismatches: int = 0
    gap_opens: int = 0


@dataclass(frozen=True)
class BlastRescueResult:
    sequence: Seq | None
    fallback_reason: str | None
    metadata: dict[str, str | int | float | bool | None] = field(default_factory=dict)


def _rescue_record(
    record: BlastRecord,
    hits: Sequence[BlastHit],
    marker: Marker,
    seed_lengths: dict[str, int],
    config: BlastRescueConfig,
) -> BlastRescueResult:
    hit_list = list(hits)
    if not hit_list:
        return BlastRescueResult(sequence=None, fallback_reason="no_blast_hit")

    thresholds = config.its2 if marker is Marker.ITS2 else config.its
    evaluated: list[tuple[BlastHit, bool, str | None, dict[str, str | int | float | bool | None]]] = []
    for hit in hit_list:
        query_start = min(hit.qstart, hit.qend)
        query_end = max(hit.qstart, hit.qend)
        query_forward = hit.qend >= hit.qstart
        subject_forward = hit.send >= hit.sstart
        strand = "+" if query_forward == subject_forward else "-"
        subject_coverage = None
        fallback_reason = None

        seed_length = seed_lengths.get(hit.subject_id)
        if seed_length is None or seed_length <= 0:
            fallback_reason = "blast_seed_unknown"
        else:
            subject_start = min(hit.sstart, hit.send)
            subject_end = max(hit.sstart, hit.send)
            subject_coverage = (subject_end - subject_start + 1) / seed_length
            query_span_length = query_end - query_start + 1
            length_ratio = query_span_length / seed_length
            endpoint_margin = max(
                config.endpoint_margin_bases,
                seed_length * config.endpoint_margin_fraction,
            )

            if subject_coverage < thresholds.min_subject_coverage:
                fallback_reason = "blast_subject_coverage_low"
            elif hit.identity / 100 < thresholds.min_identity:
                fallback_reason = "blast_identity_low"
            elif subject_start > endpoint_margin or subject_end < seed_length - endpoint_margin:
                fallback_reason = "blast_seed_endpoint_miss"
            elif (
                length_ratio < thresholds.min_query_length_ratio
                or length_ratio > thresholds.max_query_length_ratio
            ):
                fallback_reason = "blast_query_length_ratio"

        metadata = {
            "blast_seed_accession": hit.subject_id,
            "blast_identity": hit.identity,
            "blast_subject_coverage": subject_coverage,
            "blast_query_start": query_start,
            "blast_query_end": query_end,
            "blast_bitscore": hit.bitscore,
            "blast_strand": strand,
        }
        evaluated.append((hit, fallback_reason is None, fallback_reason, metadata))

    accepted = [candidate for candidate in evaluated if candidate[1]]
    if not accepted:
        _hit, _accepted, fallback_reason, metadata = max(
            evaluated,
            key=lambda candidate: candidate[0].bitscore,
        )
        return BlastRescueResult(sequence=None, fallback_reason=fallback_reason, metadata=metadata)

    accepted.sort(
        key=lambda candidate: (
            candidate[0].bitscore,
            candidate[0].identity,
            candidate[3]["blast_subject_coverage"] or 0.0,
        ),
        reverse=True,
    )
    best_hit, _accepted, _fallback_reason, best_metadata = accepted[0]
    for other in accepted[1:]:
        other_hit = other[0]
        if other_hit.bitscore < best_hit.bitscore * config.ambiguous_bitscore_ratio:
            continue
        best_start, best_end = sorted((best_hit.qstart, best_hit.qend))
        other_start, other_end = sorted((other_hit.qstart, other_hit.qend))
        overlap = max(0, min(best_end, other_end) - max(best_start, other_start) + 1)
        shorter = min(best_end - best_start + 1, other_end - other_start + 1)
        overlap_ratio = 0.0 if shorter <= 0 else overlap / shorter
        if overlap_ratio < config.ambiguous_overlap_ratio:
            return BlastRescueResult(
                sequence=None,
                fallback_reason="ambiguous_blast_hit",
                metadata=best_metadata,
            )

    start = min(best_hit.qstart, best_hit.qend) - 1
    end = max(best_hit.qstart, best_hit.qend)
    extracted = Seq(str(record.sequence)[start:end])
    if best_metadata["blast_strand"] == "-":
        extracted = extracted.reverse_complement()
    return BlastRescueResult(sequence=extracted, fallback_reason=None, metadata=best_metadata)


class BlastRunner:
    def __init__(self, config: BlastRescueConfig | None = None):
        self.config = config or BlastRescueConfig()

    def rescue(
        self,
        failed_records: Sequence[BlastRecord],
        seeds: Sequence[BlastRecord],
        marker: Marker,
    ) -> dict[str, BlastRescueResult]:
        if not failed_records:
            return {}
        if not seeds:
            return {
                record.accession_version: BlastRescueResult(
                    sequence=None,
                    fallback_reason="no_blast_seed",
                )
                for record in failed_records
            }
        if shutil.which(MAKEBLASTDB_COMMAND) is None or shutil.which(BLASTN_COMMAND) is None:
            return _failed_results(failed_records, "blast_tool_unavailable")

        with tempfile.TemporaryDirectory(prefix="barcode-kit-blast-") as temp_dir_name:
            temp_dir = Path(temp_dir_name)
            seed_path = temp_dir / "seeds.fasta"
            query_path = temp_dir / "queries.fasta"
            db_path = temp_dir / "seeds_db"
            seed_path.write_text(
                "".join(format_fasta_record(seed.accession_version, seed.sequence) for seed in seeds),
                encoding="utf-8",
            )
            query_path.write_text(
                "".join(format_fasta_record(record.accession_version, record.sequence) for record in failed_records),
                encoding="utf-8",
            )

            make_db = subprocess.run(
                [
                    MAKEBLASTDB_COMMAND,
                    "-in",
                    str(seed_path),
                    "-dbtype",
                    "nucl",
                    "-out",
                    str(db_path),
                ],
                check=False,
                capture_output=True,
                text=True,
            )
            if make_db.returncode != 0:
                return _failed_results(failed_records, "makeblastdb_failed")

            blast = subprocess.run(
                [
                    BLASTN_COMMAND,
                    "-query",
                    str(query_path),
                    "-db",
                    str(db_path),
                    "-outfmt",
                    BLASTN_OUTFMT,
                    "-dust",
                    self.config.blastn_dust,
                    "-word_size",
                    str(self.config.word_size),
                    "-evalue",
                    str(self.config.evalue),
                ],
                check=False,
                capture_output=True,
                text=True,
            )
            if blast.returncode != 0:
                return _failed_results(failed_records, "blastn_failed")

        grouped: dict[str, list[BlastHit]] = defaultdict(list)
        for line in blast.stdout.splitlines():
            if not line.strip():
                continue
            fields = line.split("\t")
            if len(fields) != 12:
                continue
            hit = BlastHit(
                query_id=fields[0],
                subject_id=fields[1],
                identity=float(fields[2]),
                alignment_length=int(fields[3]),
                mismatches=int(fields[4]),
                gap_opens=int(fields[5]),
                qstart=int(fields[6]),
                qend=int(fields[7]),
                sstart=int(fields[8]),
                send=int(fields[9]),
                evalue=float(fields[10]),
                bitscore=float(fields[11]),
            )
            grouped[hit.query_id].append(hit)

        seed_lengths = {seed.accession_version: len(str(seed.sequence)) for seed in seeds}
        results: dict[str, BlastRescueResult] = {}
        for record in failed_records:
            results[record.accession_version] = _rescue_record(
                record,
                grouped.get(record.accession_version, []),
                marker,
                seed_lengths,
                self.config,
            )
        return results


def _failed_results(
    failed_records: Sequence[BlastRecord],
    fallback_reason: str,
) -> dict[str, BlastRescueResult]:
    return {
        record.accession_version: BlastRescueResult(sequence=None, fallback_reason=fallback_reason)
        for record in failed_records
    }
